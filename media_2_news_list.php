...
if($author!=''){ //author 변수가 있을 경우 author_no를 찾는다
	$sql = "SELECT * from e_authorlist WHERE author_name = '$author'";
	$query = mysql_query($sql);
	$result = mysql_fetch_array($query);
...
<a class="newslist_a" href='media.php?media=<?=urlencode($media_name);?>' id="newsitem<?=$count?>"><div class="article_list">
	...
	<div class="article_info_12" style="width:43%;height:96px;margin:0px;border:0px solid pink;vertical-align:top;">
		<div class="newslist_newsbox_in" style="width:100%;float:left;white-space:nowrap;overflow: hidden;text-overflow:ellipsis;padding:0px;margin-left:10px;">
				<div class="newslist_newsbox_in_in" style="margin-top:4px;"> 
					...
					<div class="newslist_badge" style="display:table-cell;vertical-align:top;height:14px;">
						<div style="display:inline-block;padding-top:0px;border:0px solid blue;height:9px;float:left"><img src="img/ico_fb.png" style="width:9px;height:9px;margin: 0px 2px 0px 0px;"></div>
						<div style="display:inline-block;padding-top:0px;border:0px solid blue;height:9px;float:left"><?=number_format($author_facebook_no);?></div>
						<div style="display:inline-block;padding-top:0px;border:0px solid blue;height:9px;float:left;margin-left:4px"><img src="img/ico_str.png" style="width:9px;height:9px;margin: 0px 2px 0px 0px;"></div>
...
<div style="width:100%;height:40px;margin-top:0px;padding-top:6px;background-color:#fcfcfc">
	<div style="height:31px;float:left">
		...
		<div style="display:table-cell;height:20px;font-weight:bold;padding-top:9px;vertical-align:top">기사목록</div>
	</div>
</div>
...
<? // 목록출력부
if($media_author_no==''){
	$sql = "SELECT * from z_media_$media_no WHERE $period_array[$periodno] ORDER BY total_value";
}else{
	$sql = "SELECT * from z_media_$media_no WHERE author_no='$media_author_no' AND $period_array[$periodno] ORDER BY total_value";
}
$sql = $sql." LIMIT 0, 100";
...
d3.json("../rssdb/data.php?dateint=<?=$date_int;?>&media=<?=$media_no;?>&author=<?=$media_author_no;?>", function(dataset){
	console.log(dataset);
