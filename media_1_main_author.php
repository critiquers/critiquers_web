...
<div style="width:100%;height:40px;margin-top:0px;padding-top:6px;background-color:#fcfcfc">
	<div style="height:31px;float:left">
		...
		<div style="display:table-cell;height:20px;font-weight:bold;padding-top:9px;vertical-align:top">글쓴이 순위 (<?=$date_int;?> 이전 <?=$period_string_array[$periodno];?>)</div>
	</div>
</div>
<?
$count=1;
$i=0;
$minimum_count = 2;
$sql = "SELECT *, avg(rating), count(rating) from c_rating WHERE author_no != '0' AND $period_array[$periodno] GROUP BY author_no having count(rating)>=$minimum_count ORDER BY avg(rating) desc, count(rating) desc";
...
while($print = mysql_fetch_array($result)){ //언론사 목록을 뿌려준다
		...
		$sql2 = "SELECT media_name from e_medialist WHERE no='$media_no'";
		...
		$sql3 = "SELECT author_name from e_authorlist WHERE no='$author_no'";
		$result3 = mysql_query($sql3) or die(mysqli_error($this->db_link));
		$result_array3 = mysql_fetch_array($result3);
		$author_name = $result_array3[0];

		$sql4 = "SELECT sum(facebook_no) AS facebook_sum, sum(twitter_no) as twitter_sum from h_snscount_author WHERE $period_array[$periodno] AND author_no='$author_no'";
		$result4 = mysql_query($sql4) or die(mysqli_error($this->db_link));
		$result_array4 = mysql_fetch_array($result4);
		$facebook_no = $result_array4[0];
		$twitter_no = $result_array4[1];
?>
...
<? } ?>
<a class="newslist_a" href='media.php?media=<?=urlencode($media_name);?>&author=<?=urlencode($author_name);?>' id="newsitem<?=$count?>"><div class="article_list" t="<?=$t;?>" articleno="<?=$articleno;?>" categoryno="<?=$category;?>" belong="<?=$media;?>" author="<?=$author;?>" mediano="<?=$media_no;?>" authorno="<?=$author_no;?>" chartid="<?=$articleno."_1_".$t;?>" dateint="<?=$article_date_int;?>">
	...
	<div class="article_info_13" style="width:27%;border:0px solid purple;padding:4px 10px 4px 0px;vertical-align:top">
		<div id="media_chart_2_<?=$i;?>" class="newslist_newsbox" style="width:100%;height:70px;float:left;margin-top:10px;margin-bottom:6px;text-align:center"></div>
	</div>
</div></a>
...
$(document).ready(function(){ //탭 클릭할 떄 submit 하기
	$("#media_chart_2_<?=$i;?>").html('');
})
var outerwidth2 = $("#media_chart_2_<?=$i;?>").width();
...
var svgcont<?=$i;?> = d3.select("#media_chart_2_<?=$i;?>").append("svg")
	.attr("width", width2 + margin2.left + margin2.right)
	.attr("height", height2 + margin2.top + margin2.bottom)
	.append("g")
	.attr("transform", "translate(" + margin2.left + "," + margin2.top + ")");

d3.json("../rssdb/data_author_eval.php?author_no=<?=$author_no;?>&dateint=<?=$date_int;?>&periodno=<?=$periodno;?>", function(dataset){
	console.log(dataset);
...
<div style="margin:10px 20px 10px 20px;font-size:11px;line-height:1.3em">※ 저자의 이름이 함께 수집되는 기사에 한해 저자에 대한 평가가 누적됩니다.</div>

<div style="margin:10px 20px 20px 20px;font-size:11px">※ 통계상의 유효성을 위해 <?=$minimum_count;?>회 이상 평가가 누적된 글쓴이만 포함되었습니다.</div>
