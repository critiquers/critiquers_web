...
jindo.$Event = function(e){
    ...
    if (!jindo.$Jindo.isUndefined(e.relatedTarget)) {
        ...
        this.relatedElement = e[(this.type=="mouseout")?"to"+tElement:"from"+tElement];
    }
};
var _eventHook = {};

jindo.$Event.hook = function(sName, vRevisionEvent){
    ...
    switch(oArgs+""){
        case "g":
            return _eventHook[oArgs.sName.toLowerCase()];
        case "s4var":
            if(vRevisionEvent == null){
                delete _eventHook[oArgs.sName.toLowerCase()]; 
            }else{
                _eventHook[oArgs.sName.toLowerCase()] = vRevisionEvent;
            }
            return this;
        case "s4obj":
            var oObj = oArgs.oObj;
            for(var i in oObj){
                _eventHook[i.toLowerCase()] = oObj[i];
            }
            return this;
    }
};
...
function _releaseEventHandlerForAllChildren(wel){
...
jindo.$Element.prototype.html = function(sHTML) {
    ...
    switch(oArgs+""){
        ...
        case "s4num":
        case "s4bln":
            _releaseEventHandlerForAllChildren(this);
...
jindo.$Element.prototype.attach = function(sEvent, fpCallback){
    ...
    return this._add("normal",sEvent,null,fpCallback);
};
...
jindo.$Element.prototype.detach = function(sEvent, fpCallback){
    ...
    return this._del("normal",sEvent,null,fpCallback);
};
           ...
           if(/^ms/i.test(realEvent)){
            ...
            if (sEvent == "domready" || sEvent == "domcontentloaded") {
                ...
                if(document.body.style.WebkitTransition !== undefined){
                    ...
                    sEvent = "otrantion"+sPostfix.toLowerCase();
                ...
                if(document.body.style.WebkitAnimationName !== undefined){
                    ...
                    sEvent = "oanimation"+sPostfix.toLowerCase();
...
jindo.$Element.prototype.appear = function(duration, callback) {
    ...
    var bindFunc = function(){
        ...
        ele.style[name + 'Timing'+tFunction] = '';
        ele.style.opacity = '';
    ...
    ele.style[name + 'Timing'+tFunction] = 'linear';
...
jindo.$Element.prototype.data = function(sKey, vValue) {
    ...
    switch(oArgs+""){
        ...
        case "s4obj":
            for(var i in sKey){
                if(isNull(sKey[i])){
                    delete this._element.dataset[i];
                }else{
                    sToStr = jindo.$Json._oldToString(sKey[i]);
                    if(sToStr!=null){
                        this._element.dataset[i] = sToStr;  
                    }
                }
            }
            return this;
...
/**
 {{addEngine}}
 */
jindo.$Template.addEngine = function(sEngineName, fnEngine){
    var oArgs = g_checkVarType(arguments, {
        "4fun" : ["sEngineName:"+tString, "fnEngine:"+tFunction]
    }, "$Template#addEngine");
    
    jindo.$Template._aEngines[oArgs.sEngineName] = oArgs.fnEngine;
}
...
jindo.$Template.addEngine("default", function(sTemplate){
    ...
    // no pattern
    if(i < 2){
        return new Function("return '" + tpl[0] + "';");
    }
...
jindo.$Template.addEngine("handlebars", function(sTemplate){
    if(typeof Handlebars == "undefined"){
        throw new jindo.$Error(jindo.$Except.NOT_FOUND_HANDLEBARS, "$Template#addEngine");
    }
